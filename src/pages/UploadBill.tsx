...
const UploadBill = () => {
  ...
  const onSubmit = async (data: FormValues) => {
    if (!user) {
      toast({ title: "Unauthorized", description: "You must be logged in to upload a bill", variant: "destructive" });
      return;
    }
    ...
    if (!data.title.trim()) {
      toast({ title: "Missing Title", description: "Please enter a transaction title", variant: "destructive" });
      return;
    }
    ...
    if (selectedFile) {
      try {
        const fileRef = ref(storage, `user_uploads/${user.uid}/${Date.now()}_${selectedFile.name}`);
        const snapshot = await uploadBytes(fileRef, selectedFile);
        fileURL = await getDownloadURL(snapshot.ref);
      } catch (error) {
        toast({ title: "File Upload Failed", description: "Could not upload the file", variant: "destructive" });
        return;
      }
    }
    ...
    try {
      ...
      toast({ title: "Error", description: "Something went wrong while saving the bill", variant: "destructive" });
    }
  };
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                {entryType === 'file' && (
                  <div className="space-y-2">
                    ...
                    />
                    <FileUploadPreview file={selectedFile} onClear={() => setSelectedFile(null)} />
                  </div>
                )}

                <div className="space-y-2">
                  <Label htmlFor="title" className="text-base">Transaction Title *</Label>
                  <Input id="title" type="text" placeholder="Enter transaction title" {...register("title", { required: true })} />
                  {errors.title && <p className="text-sm text-destructive">Transaction title is required</p>}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    {errors.amount && <p className="text-sm text-destructive">Please enter a valid amount</p>}
                  </div>
                ...
                <Button type="submit" className="w-full py-6 text-lg mt-8">Save Transaction</Button>
              </form>
            </CardContent>
          </Card>
        </div>
            ...
            <CardContent className="p-6">
              <div className="space-y-4">
                ...
                <div className="mt-8 bg-gray-50 p-4 rounded-lg">
                  ...
                  <p className="text-sm text-muted-foreground">All transactions are saved and appear in reports automatically.</p>
                </div>
              </div>
            </CardContent>
          </Card>
